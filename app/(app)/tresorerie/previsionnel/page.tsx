...
import { construirePlan, indiceMois, moisSuivants, type Flux } from '@/lib/prevision'
import { CATEGORIE_CREANCES, CATEGORIE_DETTES, CATEGORIE_ECHEANCES, CATEGORIES_PREVISION } from '@/lib/tresorerie-previsionnelle'
...
export default async function PrevisionnelPage({ searchParams }: { searchParams: Promise<{ delai?: string }> }) {
  const { delai: delaiParam } = await searchParams
  ...
  const [{ data: soldes }, { data: echeances }, { data: tiers }, { data: previsions }] = await Promise.all([
      ...
      >
        <SimpleCreateForm
          ...
          champs={[
            ...
            { name: 'categorie', label: t('Catégorie'), type: 'select', defaultValue: 'autre', options: Object.entries(CATEGORIES_PREVISION).map(([value, label]) => ({ value, label: t(label) })) },
