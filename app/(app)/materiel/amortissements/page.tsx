  ...
  return (
    <>
      <PageHeader
        ...
        description="Comptabilisation par exercice : dotation de chaque bien (imputée à son département d'affectation) et reprise des subventions correspondantes. Relancer un exercice ne traite que les biens non encore amortis sur cette période."
      >
      ...
      <Card className="mb-4 text-sm text-foreground-muted">
        Convention linéaire au mois : amortissement dès le mois de mise en service. À valider avec votre expert-comptable (prorata en jours, mode dégressif fiscal éventuel).
      </Card>
      <TableWrap>
        <thead>
          <tr>
            ...
            <th className={`${th} text-right`}>Biens amortis</th>
            <th className={`${th} text-right`}>Dotations</th>
            <th className={`${th} text-right`}>Reprises de subventions</th>
            <th className={th}></th>
          </tr>
        </thead>
        <tbody>
          {exercices?.map((e) => {
            const t = parExercice.get(e.id)
            return (
              <tr key={e.id}>
                <td className={td}>{e.libelle}</td>
                <td className={td}>{formatDate(e.date_debut)} → {formatDate(e.date_fin)}</td>
                <td className={`${td} text-right`}>{t?.nb ?? 0}</td>
                <td className={`${td} text-right tabular-nums`}>{formatMontant(t?.dotations ?? 0, ctx.devise)}</td>
                <td className={`${td} text-right tabular-nums`}>{formatMontant(t?.reprises ?? 0, ctx.devise)}</td>
                <td className={td}>
                  {peutEcrire && e.statut === 'ouvert' && (
                    <ActionButton
                      label="Comptabiliser"
                      confirmation={`Comptabiliser les dotations de l'exercice « ${e.libelle} » ?`}
