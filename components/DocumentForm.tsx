  ...
  return (
    <div className="space-y-4">
      <Card className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="tiers">{cfg.titreTiers}</Label>
        ...
        {(kind === 'vente' || kind === 'distribution') && (
          <>
            <div className="space-y-1.5">
              <Label htmlFor="dep">Département (produit imputé)</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="sec">Secteur / projet</Label>
        ...
        return (
          <Card key={i} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            <div className="space-y-1.5 lg:col-span-2">
              <Label htmlFor={`p-${i}`}>Produit</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor={`q-${i}`}>Quantité{p ? ` (${p.unite})` : ''}</Label>
            ...
            <div className="space-y-1.5">
              {(kind === 'vente' || kind === 'distribution') && contrats.length > 0 ? (
                <>
                  <Label htmlFor={`c-${i}`}>Dépôt-vente</Label>
                ...
                <>
                  ...
                  <p className="pt-2 text-sm tabular-nums">{formatMontant(montant, devise)}</p>
                </>
              ) : null}
            </div>
            {lignes.length > 1 && (
              <div className="flex items-end">
                <Button type="button" variant="outline" size="icon" aria-label="Supprimer la ligne"
      ...
      <Card className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        {avecPrix ? (
          <dl className="grid grid-cols-3 gap-4 text-sm">
            <div><dt className="text-foreground-muted">HT</dt><dd className="font-medium tabular-nums">{formatMontant(totaux.ht, devise)}</dd></div>
            <div><dt className="text-foreground-muted">TVA</dt><dd className="font-medium tabular-nums">{formatMontant(totaux.tva, devise)}</dd></div>
            <div><dt className="text-foreground-muted">TTC</dt><dd className="font-semibold tabular-nums">{formatMontant(totaux.ttc, devise)}</dd></div>
          </dl>
        ) : <span className="text-sm text-foreground-muted">Stock consigné : aucune écriture comptable (hors bilan).</span>}
