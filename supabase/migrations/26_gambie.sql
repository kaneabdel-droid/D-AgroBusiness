-- D-AGROBUSINESS — Gambie, étape 2 : plan comptable IFRS_GM, monnaie GMD et modèle de paie PAYE + SSHFC.
-- À exécuter APRÈS 25_gambie_referentiel.sql (validé séparément).
-- Sources : Gambia Revenue Authority (brochure « Employment Income Tax – PAYE » et calculateur PAYE du site gra.gm) et
-- SSHFC (page « National Provident Fund »). Aucun changement du moteur de retenue n'est nécessaire.
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.

-- Plan comptable IFRS_GM : plan interne à numérotation par classes, libellés en anglais, identique au plan IFRS_GH
-- (mêmes numéros que ceux utilisés par tous les modules), avec le libellé de sécurité sociale adapté à la Gambie.
insert into modeles_plan_comptable (referentiel, numero, libelle)
  select 'IFRS_GM', numero, libelle from modeles_plan_comptable where referentiel = 'IFRS_GH'
on conflict do nothing;

update modeles_plan_comptable set libelle = 'Social security (SSHFC) payable'
  where referentiel = 'IFRS_GM' and numero = '431';
...
begin
  ...
  if p_pays = 'SN' then
    ...
    -- Mauritanie (Code général des impôts 2023, art. 110, 113 et 114 — barème de la loi de finances 2022) :
    --  rémunération imposable = rémunération globale − retenues obligatoires de pension et de sécurité sociale − 6 000 MRU par mois
    --  exonérés (art. 110-1-c), arrondie à la dizaine inférieure (art. 114-3) ;
    --  barème mensuel : 15 % jusqu'à 9 000, 25 % de 9 000 à 21 000, 40 % au-delà (formules du code : 0,15 R, 0,25 R − 900, 0,40 R − 4 050,
    --  équivalentes à des tranches marginales) ; aucune réduction pour charges de famille.
    -- Cotisations (CLEISS, « Les cotisations en Mauritanie », taux au 01/01/2025) : CNSS (retraite, accidents du travail, prestations
    --  familiales) 1 % salarié + 13 % employeur, plafond 70 000 MRU/mois ; CNAM (maladie) 4 % salarié + 5 % employeur, sans plafond ;
    --  médecine du travail 2 % employeur (plafond 70 000). D'autres sources donnent un plafond CNSS de 15 000 MRU (décret 2021-186) :
    --  à vérifier auprès de la CNSS et à corriger dans les règles si nécessaire.
    -- Non gérés : indemnités exonérées jusqu'à 1 000 MRU (art. 110-1-b), avantages en nature retenus à 40 % (art. 113), taux plafonné à
    --  20 % pour les journalistes et navigants (art. 114-6). La loi de finances 2026 n'aurait pas modifié le barème (source de presse,
    --  non vérifiée dans le texte). La Mauritanie utilise le plan comptable PCM_MR et la monnaie MRU (attribués à l'inscription).
    update parametrage_paie
    ...
    -- Guinée (Code général des impôts, loi L/2021/032/AN applicable depuis le 1er janvier 2022 : art. 57-58 base, art. 63 barème,
    -- art. 201 versement forfaitaire) :
    --  revenu imposable = rémunération − cotisations salariales aux assurances sociales et retenues de retraite obligatoires ;
    --  barème mensuel (retenue libératoire) : 0 % jusqu'à 1 000 000 GNF, 5 % jusqu'à 3 000 000, 8 % jusqu'à 5 000 000, 10 % jusqu'à
    --  10 000 000, 15 % jusqu'à 20 000 000, 20 % au-delà (montants saisis × 12) ; aucune réduction pour charges de famille ;
    --  versement forfaitaire sur les salaires (VF) : 6 % de la masse salariale à la charge de l'employeur.
    -- CNSS (site cnss.gov.gn) : 23 % dont 5 % salarié et 18 % employeur (prestations familiales 6 %, accidents du travail 4 %,
    --  assurance maladie 6,5 %, retraite 6,5 % dont 5 % salarié), assiette plafonnée à 2 500 000 GNF par mois.
    -- Non gérés : plancher de l'assiette CNSS (SMIG, 550 000 GNF) ; exonération des indemnités forfaitaires (logement, transport,
    --  panier, cherté de vie… dans la limite de 25 % du salaire brut, art. 55-3) : à déduire du brut par les règles ou primes non
    --  imposables ; VF calculé sur le brut, alors que le code en déduit les cotisations de prestations familiales (écart de 6 % de
    --  la cotisation PF, à ajuster). La Guinée utilise SYSCOHADA et la monnaie GNF (attribuées à l'inscription).
    update parametrage_paie
    ...
    -- Nigeria (Nigeria Tax Act 2025, en vigueur au 1er janvier 2026 : art. 30 déductions, 4e annexe barème) :
    --  la retenue (PAYE) est calculée sur l'année (salaire mensuel × 12) puis ramenée au mois ;
    --  revenu imposable = revenu total − déductions admises : cotisation de retraite (Pension Reform Act), Fonds national du logement
    --  (NHF), assurance santé nationale, intérêts d'emprunt d'habitation principale, primes d'assurance-vie, et allègement de loyer de
    --  20 % du loyer annuel payé dans la limite de 500 000 NGN par an (l'ancien abattement CRA est supprimé) ;
    --  barème annuel : 800 000 à 0 %, puis 15 % jusqu'à 3 000 000, 18 % jusqu'à 12 000 000, 21 % jusqu'à 25 000 000, 23 % jusqu'à
    --  50 000 000, 25 % au-delà.
    -- Cotisations (résumé fiscal PwC, taux légaux minimaux) : retraite 8 % salarié + 10 % employeur des émoluments mensuels ;
    --  NHF 2,5 % du revenu mensuel à la charge du salarié.
    -- À saisir hors modèle : allègement de loyer (le saisir par employé en « déduction fixe mensuelle » = 1/12 de l'allègement annuel),
    --  assurance santé, intérêts d'emprunt et primes d'assurance-vie (règles déductibles ou déduction fixe). Non paramétrés : NSITF et
    --  ITF (taux non vérifiés). Les émoluments de retraite sont assimilés au salaire brut (la base légale exclut certains éléments).
    -- Le Nigeria utilise le plan comptable IFRS_NG (anglais) et la monnaie NGN (attribués à l'inscription).
    update parametrage_paie
       set deduction_forfaitaire_mensuelle = 0, abattement_seuil_annuel = null, abattement_pct_bas = 0, deduction_par_charge_annuelle = 0,
           abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 0, abattement_plafond_annuel = null,
           arrondi_base = 0, ricf_mode = 'parts', ricf_marie_pct = 0, ricf_par_enfant_pct = 0,
    ...
    -- Gambie (Gambia Revenue Authority, brochure « Employment Income Tax – PAYE », barème de janvier 2018, confirmé par le
    -- calculateur PAYE du site de la GRA ; Income and Value Added Tax Act 2012, première annexe) :
    --  l'impôt sur les revenus d'emploi (PAYE) est calculé sur l'année (salaire mensuel × 12) puis ramené au mois, sur la totalité
    --  des revenus d'emploi : « aucune déduction n'est admise » (la cotisation sociale du salarié n'est donc pas déduite) ;
    --  barème annuel : 0 % jusqu'à 36 000 GMD, 5 % jusqu'à 46 000, 10 % jusqu'à 56 000, 15 % jusqu'à 66 000, 20 % jusqu'à 76 000,
    --  25 % au-delà (équivalent à (revenu − 76 000) × 25 % + 5 000 au-dessus de 76 000).
    -- Cotisations (SSHFC, National Provident Fund) : 15 % du salaire de base, dont 5 % salarié et 10 % employeur, sans plafond.
    -- Non gérés : l'assiette SSHFC est le salaire de base (le salaire du contrat est utilisé) ; la section 44 de l'Act prévoit une
    --  déduction limitée pour cotisations à une caisse de retraite agréée (à 7 500 GMD par an selon un résumé non vérifié dans le texte),
    --  que la GRA n'applique pas au PAYE dans son brochure — non retenue.
    -- La Gambie utilise le plan comptable IFRS_GM (anglais) et la monnaie GMD (attribués à l'inscription).
    update parametrage_paie
       set deduction_forfaitaire_mensuelle = 0, abattement_seuil_annuel = null, abattement_pct_bas = 0, deduction_par_charge_annuelle = 0,
           abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 0, abattement_plafond_annuel = null,
           arrondi_base = 0, ricf_mode = 'parts', ricf_marie_pct = 0, ricf_par_enfant_pct = 0,
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR', 'GN', 'NG', 'GM') and not exists (select 1 from regles_paie where organisation_id = p_org) then
