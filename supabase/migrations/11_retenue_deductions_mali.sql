-- D-AGROBUSINESS — Moteur de retenue : déductions de la base, réduction familiale en pourcentage, réduction de points
-- de pression fiscale, et modèle Mali (source : DGI du Mali, brochure « L'impôt sur les traitements et salaires n° 2 »,
-- édition d'avril 2020, dont le cas pratique — salarié marié, 3 enfants, 175 000 F par mois, ITS mensuel de 13 770 F —
-- sert de test de non-régression).
--
-- Le Sénégal et la Côte d'Ivoire ne sont pas modifiés : leurs paramètres restent en mode « parts » sans déduction.

alter table parametrage_paie
  add column ricf_mode text not null default 'parts' check (ricf_mode in ('parts', 'familial')),
  add column ricf_marie_pct numeric(6,3) not null default 0,
  add column ricf_par_enfant_pct numeric(6,3) not null default 0,
  add column ricf_max_enfants integer not null default 10,
  add column reduction_pression_points numeric(6,3) not null default 0;

-- Déduction forfaitaire mensuelle propre à l'employé (Mali : indemnité spéciale de solidarité 2 000 / 5 000 / 6 500 F)
alter table employes
  add column deduction_fixe_mensuelle numeric(18,2) not null default 0 check (deduction_fixe_mensuelle >= 0);
...
begin
  if (new.jours_par_mois, new.jours_conge_par_mois, new.abattement_pct, new.abattement_plafond_annuel,
     ...
     (old.jours_par_mois, old.jours_conge_par_mois, old.abattement_pct, old.abattement_plafond_annuel,
      ...
      old.ricf_mode, old.ricf_marie_pct, old.ricf_par_enfant_pct, old.ricf_max_enfants, old.reduction_pression_points) then
    new.valide_le := null;
    new.valide_par := null;
  end if;
  return new;
...
-- ============================================================
-- Retenue calculée : signature étendue (déductions, situation familiale)
-- ============================================================
drop function if exists retenue_calculee(uuid, text, numeric, numeric);
...
-- ============================================================
-- Calcul des bulletins : déductions et situation familiale transmises au calcul de la retenue
-- ============================================================
