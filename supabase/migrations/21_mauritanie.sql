-- D-AGROBUSINESS — Mauritanie : modèle de paie ITS + CNSS + CNAM.
-- Sources : Code général des impôts de Mauritanie, édition 2023 (art. 109 à 114, barème de la loi de finances 2022) publié par le
-- ministère des Finances ; CLEISS (taux 2025). Le site de la DGI mauritanienne étant inaccessible, une éventuelle modification
-- postérieure (loi de finances 2026) n'a pas pu être vérifiée dans le texte.
-- Nouveauté du moteur : montant mensuel exonéré propre au pays (deduction_forfaitaire_mensuelle).
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.

alter table parametrage_paie add column if not exists deduction_forfaitaire_mensuelle numeric(18,2) not null default 0;
...
begin
  if (new.jours_par_mois, new.jours_conge_par_mois, new.abattement_pct, new.abattement_plafond_annuel,
      ...
      new.deduction_forfaitaire_mensuelle)
     is distinct from
     (old.jours_par_mois, old.jours_conge_par_mois, old.abattement_pct, old.abattement_plafond_annuel,
      ...
      old.abattement_sur_brut, old.deduction_par_charge_annuelle, old.abattement_seuil_annuel, old.abattement_pct_bas,
      old.deduction_forfaitaire_mensuelle) then
    new.valide_le := null;
    new.valide_par := null;
  end if;
  return new;
...
begin
  ...
  v_d := case when v_niveau = 'annuel' then p_deduction * v_n else p_deduction end
         -- Mauritanie : montant mensuel exonéré (art. 110-1-c), appliqué aux paies mensuelles
         + case when par.deduction_forfaitaire_mensuelle > 0 and p_periodicite = 'mensuel'
                then par.deduction_forfaitaire_mensuelle * case when v_niveau = 'annuel' then v_n else 1 end else 0 end;
...
begin
  if p_pays not in ('SN', 'CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR') then
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR') and not exists (select 1 from regles_paie where organisation_id = p_org) then
