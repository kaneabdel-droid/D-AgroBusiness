-- D-AGROBUSINESS — Burkina Faso : modèle de paie IUTS + CNSS + TPA.
-- Sources : Code général des impôts (recueil de textes fiscaux de la DGI, éd. 2014 : art. 60 à 62 et 122 à 124 ; réforme de 2018
-- sur les charges de famille confirmée par la presse burkinabè et le blog fiscal cité dans le suivi) et site de la CNSS (cnssbf.org).
-- Nouveautés du moteur : mode de réduction familiale « charges_impot » (pourcentage de l'impôt selon le nombre de charges) et
-- abattement pour frais professionnels calculé sur le salaire avant déduction de la pension (abattement_sur_brut).
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.

alter table parametrage_paie add column if not exists abattement_sur_brut boolean not null default false;
...
alter table parametrage_paie
  add constraint parametrage_paie_ricf_mode_check check (ricf_mode in ('parts', 'familial', 'abattement_base', 'charges_impot'));
...
begin
  if p_pays not in ('SN', 'CI', 'ML', 'NE', 'BJ', 'GH', 'BF') then
  ...
  if p_pays = 'SN' then
    update parametrage_paie
       set abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, ricf_mode = 'parts', reduction_pression_points = 0, abattement_pct = 30, abattement_plafond_annuel = 900000,
           arrondi_base = 1000,
    ...
    update parametrage_paie
       set abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, ricf_mode = 'parts', reduction_pression_points = 0, abattement_pct = 0, abattement_plafond_annuel = null,
           arrondi_base = 0,
    ...
    update parametrage_paie
       set abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 0, abattement_plafond_annuel = null,
    ...
    update parametrage_paie
       set abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 10, abattement_plafond_annuel = null,
    ...
    update parametrage_paie
       set abattement_sur_brut = true, mode_ir = 'calcul', bareme_version = null, abattement_pct = 25, abattement_plafond_annuel = null,
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF') and not exists (select 1 from regles_paie where organisation_id = p_org) then
