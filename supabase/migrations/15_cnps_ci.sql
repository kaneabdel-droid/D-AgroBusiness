-- D-AGROBUSINESS — Côte d'Ivoire : cotisations CNPS (retraite, prestations familiales, maternité, accidents du travail).
-- Sources officielles : CNPS, communiqué « Nouveau plafond des cotisations sociales » (SMIG 75 000 F au 01/01/2023) et
-- brochure « Recouvrement » (taux). Les organisations ivoiriennes déjà créées rechargent le modèle depuis
-- RH > Paramètres (« Charger le modèle »), puis revalident le paramétrage.
