-- D-AGROBUSINESS — Nigeria, étape 2 : plan comptable IFRS_NG, monnaie NGN et modèle de paie PAYE.
-- À exécuter APRÈS 23_nigeria_referentiel.sql (validé séparément).
-- Sources : Nigeria Tax Act 2025 (texte de la loi : art. 30 et 4e annexe), résumés PwC et KPMG (barème, taux de cotisation).
-- Aucun changement du moteur de retenue n'est nécessaire. L'interface reste en français à ce stade.
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.

-- Plan comptable IFRS_NG : plan interne à numérotation par classes, libellés en anglais, identique au plan IFRS_GH
-- (mêmes numéros que ceux utilisés par tous les modules), avec les libellés de tiers adaptés au Nigeria.
insert into modeles_plan_comptable (referentiel, numero, libelle)
  select 'IFRS_NG', numero, libelle from modeles_plan_comptable where referentiel = 'IFRS_GH'
on conflict do nothing;

update modeles_plan_comptable set libelle = 'Pension, housing fund and other social contributions payable'
  where referentiel = 'IFRS_NG' and numero = '431';
...
begin
  ...
  v_ref := case v_pays when 'MA' then 'PCM_MA' when 'MR' then 'PCM_MR' when 'GH' then 'IFRS_GH' when 'NG' then 'IFRS_NG' else 'SYSCOHADA' end;
  v_devise := case v_pays when 'MA' then 'MAD' when 'MR' then 'MRU' when 'GH' then 'GHS' when 'GN' then 'GNF' when 'NG' then 'NGN' else 'XOF' end;
...
language plpgsql security definer set search_path = public as $$
begin
  if p_pays not in ('SN', 'CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR', 'GN', 'NG') then
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR', 'GN', 'NG') and not exists (select 1 from regles_paie where organisation_id = p_org) then
