-- D-AGROBUSINESS — Togo : modèle de paie IRPP + CNSS + assurance maladie universelle.
-- Sources : Code général des impôts du Togo, mise à jour 2023, publié par l'OTR (art. 26, 72-74) ; CLEISS (taux CNSS 2026) ;
-- décret 2023-096/PR fixant les taux de cotisation de l'assurance maladie universelle (art. 12), lu sur le site de la CNSS du Togo.
-- Nouveauté du moteur : mode de charges de famille « deduction_charge » (montant fixe annuel déduit du revenu net par personne à charge).
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.

alter table parametrage_paie add column if not exists deduction_par_charge_annuelle numeric(18,2) not null default 0;
...
alter table parametrage_paie
  ...
  check (ricf_mode in ('parts', 'familial', 'abattement_base', 'charges_impot', 'deduction_charge'));
...
begin
  if (new.jours_par_mois, new.jours_conge_par_mois, new.abattement_pct, new.abattement_plafond_annuel,
      ...
      new.abattement_sur_brut, new.deduction_par_charge_annuelle)
     is distinct from
     (old.jours_par_mois, old.jours_conge_par_mois, old.abattement_pct, old.abattement_plafond_annuel,
      ...
      old.abattement_sur_brut, old.deduction_par_charge_annuelle) then
    new.valide_le := null;
    new.valide_par := null;
  end if;
  return new;
...
begin
  ...
  v_net := greatest(0, v_net - least(par.abattement_pct / 100 * case when par.abattement_sur_brut then v_b else v_net end,
...
begin
  if p_pays not in ('SN', 'CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG') then
  ...
  if p_pays = 'SN' then
    update parametrage_paie
       set deduction_par_charge_annuelle = 0, abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, ricf_mode = 'parts', reduction_pression_points = 0, abattement_pct = 30, abattement_plafond_annuel = 900000,
           arrondi_base = 1000,
    ...
    update parametrage_paie
       set deduction_par_charge_annuelle = 0, abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, ricf_mode = 'parts', reduction_pression_points = 0, abattement_pct = 0, abattement_plafond_annuel = null,
           arrondi_base = 0,
    ...
    update parametrage_paie
       set deduction_par_charge_annuelle = 0, abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 0, abattement_plafond_annuel = null,
    ...
    update parametrage_paie
       set deduction_par_charge_annuelle = 0, abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null, abattement_pct = 10, abattement_plafond_annuel = null,
    ...
    update parametrage_paie
       set deduction_par_charge_annuelle = 0, abattement_sur_brut = true, mode_ir = 'calcul', bareme_version = null, abattement_pct = 25, abattement_plafond_annuel = null,
    ...
    update parametrage_paie
       set deduction_par_charge_annuelle = 120000, abattement_sur_brut = false, mode_ir = 'calcul', bareme_version = null,
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG') and not exists (select 1 from regles_paie where organisation_id = p_org) then
