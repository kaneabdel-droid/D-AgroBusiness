-- D-AGROBUSINESS — Guinée : modèle de paie RTS + CNSS + versement forfaitaire, et monnaie GNF à l'inscription.
-- Sources : Code général des impôts de la Guinée (loi L/2021/032/AN, applicable depuis le 01/01/2022 : art. 57, 58, 63, 201),
-- publié par la DGI, et site de la CNSS de Guinée (taux et plafond). Aucun changement du moteur de retenue n'est nécessaire.
-- Les organisations existantes chargent le modèle depuis RH > Paramètres, puis revalident.
-- Les montants en GNF s'affichent avec deux décimales (seul le franc CFA est présenté sans décimales).
...
begin
  ...
  v_devise := case v_pays when 'MA' then 'MAD' when 'MR' then 'MRU' when 'GH' then 'GHS' when 'GN' then 'GNF' else 'XOF' end;
...
language plpgsql security definer set search_path = public as $$
begin
  if p_pays not in ('SN', 'CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR', 'GN') then
...
begin
  ...
  if v_pays in ('CI', 'ML', 'NE', 'BJ', 'GH', 'BF', 'TG', 'MA', 'MR', 'GN') and not exists (select 1 from regles_paie where organisation_id = p_org) then
