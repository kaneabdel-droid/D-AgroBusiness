...
## Super-administration et démonstration

- **/admin** (réservé aux emails de `ADMIN_EMAILS`, identité partagée avec les autres produits DembaSolution) : tableau de bord de la plateforme, entreprises (niveau, échéance, abonnement accordé à la main, prolongation d'essai, verrouillage), paiements, configuration (moyens de paiement proposés aux clients : Chariow actif, Bictorys et Moneroo prêts en attente de leurs clés ; produits Chariow par niveau et durée ; adresses des webhooks). Migration 33.
- **Démonstration** : `npm run seed:demo` crée l'entreprise « Riz du Delta » (campagnes, achats, production, usine, ventes, financement, paie, traçabilité, prévisions, rapprochement). Le lien « Découvrir » (`/decouvrir-dagrobusiness`) ouvre une session sans mot de passe sur les comptes direction, comptable ou RH.
- **Vitrine** : `/bienvenue` (accueil), `/tarifs`, `/decouvrir-dagrobusiness`. Un visiteur non connecté sur `/` est redirigé vers l'accueil. Fiche à reprendre sur la landing DembaSolution : `docs/fiche-produit-landing-dembasolution.md`.
- **Audit** : `docs/AUDIT.md`.
